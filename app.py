...
# Sidebar Theme Selector Switch
theme_mode = st.sidebar.toggle("🌙 Dark Theme", value=True)
...
# Inject styles matching user reference pages, safe for dark/light transitions
st.markdown(f"""
<style>
    ...
    /* Force sidebar text to follow theme colors */
    ...
    .method-card {{
        background-color: {method_bg} !important;
        border-radius: 8px;
        padding: 15px;
        margin-bottom: 10px;
...
# Helper function to display dictionary visually (Pure Python, instant rendering)
...
# ----------------- NATIVE VISUALIZER MODULE (Dictionary Only) -----------------
elif page == "visualizer":
    # Rebuilt visualizer logic natively to make it fast
    if 'demo_dict' not in st.session_state:
        st.session_state.demo_dict = {"Name": "Riya", "RollNo": 1, "Status": "Active"}

    st.title("⚡ Dictionary Operations Visualizer")
    
    st.markdown("""
    <div class="info-box">
        <h3>Memory Mapping in Action</h3>
        <p>Watch how Python maps unique keys to values. Adding a key that already exists will
        <strong>update</strong> its value, demonstrating the uniqueness property of dictionary keys.</p>
    </div>
    """, unsafe_allow_html=True)

    col_actions, col_visual = st.columns([2, 3])

    with col_actions:
        st.markdown("### Operations / Controls")
        
        with st.expander("➕ Add / Update Key-Value Pair", expanded=True):
            add_key = st.text_input("Enter Key:", "City")
            add_val = st.text_input("Enter Value:", "Delhi")
            if add_val.isdigit():
                add_val = int(add_val)
            
            if st.button("Apply D[key] = value", use_container_width=True):
                if add_key:
                    st.session_state.demo_dict[add_key] = add_val
                    st.success(f"Executed: D['{add_key}'] = {repr(add_val)}")
                    time.sleep(0.5)
                    st.rerun()
                    
        with st.expander("🗑️ Delete Key"):
            if st.session_state.demo_dict:
                del_key = st.selectbox("Select Key to delete:", list(st.session_state.demo_dict.keys()))
                if st.button("Apply D.pop(key)", use_container_width=True):
                    val = st.session_state.demo_dict.pop(del_key)
                    st.success(f"Executed: D.pop('{del_key}') -> returned {repr(val)}")
                    time.sleep(0.5)
                    st.rerun()
            else:
                st.info("Dictionary is empty, nothing to delete.")
                
        if st.button("🚨 Reset Dictionary", use_container_width=True):
            st.session_state.demo_dict = {"Name": "Riya", "RollNo": 1, "Status": "Active"}
            st.success("Dictionary reset to initial state!")
            time.sleep(0.5)
            st.rerun()

    with col_visual:
        st.markdown("### Live Internal State")
        items_str = ", ".join([f"'{k}': {repr(v)}" for k, v in st.session_state.demo_dict.items()])
        st.code(f"my_dict = {{{items_str}}}", language="python")
        render_dictionary(st.session_state.demo_dict)

# ----------------- NATIVE CODE LAB MODULE (Dictionary Only) -----------------
elif page == "codelab":
        ...
        with col_btn1:
            if st.button("▶ Step Forward", use_container_width=True):
                if st.session_state.loop_idx < len(keys_list) - 1:
                    ...
                    st.session_state.loop_idx = len(keys_list)
        with col_btn2:
...
# ----------------- NATIVE PROGRAMS MODULE (Dictionary Only) -----------------
elif page == "programs":
